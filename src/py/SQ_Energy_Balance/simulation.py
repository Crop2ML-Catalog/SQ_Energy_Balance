...
def simulation(datafile, vardata, params, init):
    ...
    for i in range(0,len(df.index)-1):
        ...
        VPDair = t_VPDair[i]
        netRadiation,netOutGoingLongWaveRadiation,netRadiationEquivalentEvaporation,evapoTranspirationPriestlyTaylor,diffusionLimitedEvaporation,energyLimitedEvaporation,conductance,evapoTranspirationPenman,soilEvaporation,evapoTranspiration,potentialTranspiration,soilHeatFlux,cropHeatFlux,minCanopyTemperature,maxCanopyTemperature= EnergyBalanceComponent.model_energybalance(minTair,maxTair,albedoCoefficient,stefanBoltzman,elevation,solarRadiation,vaporPressure,extraSolarRadiation,lambdaV,hslope,psychrometricConstant,Alpha,vonKarman,heightWeatherMeasurements,zm,d,zh,plantHeight,wind,deficitOnTopLayers,soilDiffusionConstant,VPDair,rhoDensityAir,specificHeatCapacityAir,tau,tauAlpha,isWindVpDefined)
